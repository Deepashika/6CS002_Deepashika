...
public class Main {
  ...
  private boolean topLeftH(int x, int y) {
    Domino e = findDominoAt(x, y + 1);
    return thisIsTopLeftOfDomino(x, y + 1, e) && e.ishl();
  }

  private boolean thisIsTopLeftOfDomino(int x, int y, Domino d) {
	  boolean retur_minMathx = (x == Math.min(d.lx, d.hx));
	  boolean retur_minyMathy = (y == Math.min(d.ly, d.hy));
	  return  retur_minMathx  && retur_minyMathy;
  }
...
//    System.out.println("Serial number " + Special.getStamp());
    ...
    while (_$_ != ZERO) {
      ...
      case 3: {
        ...
        JFrame f = new JFrame("Rules by Julio Sanchez");
